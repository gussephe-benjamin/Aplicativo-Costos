...
@materia_prima_bp.route('/materia-prima/<int:id>', methods=['GET'])
...
def create_materia_prima():
    ...
    nuevo_registro = crear_materia_prima(nombre, cantidad_disponible, precio_por_unidad)
    return jsonify(nuevo_registro), 201

# Ruta para actualizar un registro de materia prima
@materia_prima_bp.route('/materia-prima/<int:id>', methods=['PUT'])
...
@materia_prima_bp.route('/materia-prima/<int:id>', methods=['DELETE'])
